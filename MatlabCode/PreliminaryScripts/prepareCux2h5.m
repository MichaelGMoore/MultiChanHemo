% Prepare Cux2 GFP data as .h5 with simple schema
...
folder = '/home/mmoore/Optical_Data_Analysis/Data/Widefield/Matt_Valley/GFP_Mice/Cux2/Raw';
fdir = dir(folder);
fdir = fdir(3:end);
...
for m = 1:length(fdir)
    ...
    % for cux2 we want to apply time-shifts: [0,4,2] (F1,R1,R2)
    F = F(:,:,1:(end-4));
    R1 = R1(:,:,5:end);
    R2 = R2(:,:,3:(end-2));
    ...
    outFolder = '/home/mmoore/Dropbox/Work/projects/WFOM_MultiChanHemo/Version3/PreliminaryScripts/Cux2';
    ...
    h5writeatt(fname,'/','CreName','Cux2'); % this will not be a required field
